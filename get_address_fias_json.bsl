// Получение адреса ФИАС. Веб-сервис 1С. https://api.orgaddress.1c.ru/orgaddress/v1?wsdl
...
Процедура ЗаполнитьСтруктуруАдресаИзСтроки(СтруктураАдреса, СтроковыйАдрес)
	...
	СтруктураАдреса.Вставить("addressType", "Административно-территориальный");
	СтруктураАдреса.Вставить("ZIPcode",     ДанныеСтроки.Индекс);
	
	ЗаполнитьОписаниеЧастиАдреса(СтруктураАдреса,"area",ДанныеСтроки.Регион);
	ЗаполнитьОписаниеЧастиАдреса(СтруктураАдреса,"district",ДанныеСтроки.Район);
	ЗаполнитьОписаниеЧастиАдреса(СтруктураАдреса,"city",ДанныеСтроки.Город);
	ЗаполнитьОписаниеЧастиАдреса(СтруктураАдреса,"locality",ДанныеСтроки.НаселенныйПункт);
	ЗаполнитьОписаниеЧастиАдреса(СтруктураАдреса,"street",ДанныеСтроки.Улица);
		
	СтруктураАдреса.Вставить("houseNumber",      ДанныеСтроки.Дом);
    СтруктураАдреса.Вставить("houseType",      ДанныеСтроки.ТипДома);
	
	СтруктураАдреса.Вставить("apartments", Новый Массив);
	Если ЗначениеЗаполнено(ДанныеСтроки.Улица) Тогда
		...
		СтруктураАдреса.apartments.Добавить(ВебКвартира);
	КонецЕсли;	
	
	СтруктураАдреса.Вставить("buildings", Новый Массив);
	Если ЗначениеЗаполнено(ДанныеСтроки.Корпус) Тогда	
		...
		СтруктураАдреса.buildings.Добавить(ВебКорпус);
	КонецЕсли;
КонецПроцедуры